...
class WebinarGridView extends Component {
  render() {
    ...
    return (      
			...
			{ webinarListData ?
				webinarListData.webinarList.map((item , index) => {
					return  <div className="item width-product-list grid-width-item" key={index}>
						 ...
						 <Link to ={item.status === "webinar" ? "/webinar-product/"+item.productSlug :  "/product/"+item.productSlug} className="product-inner shine-effect-inner clearfix">
								...
								<div className="product-group-info">
									 ...
									 <div className="remaing-items">
											<span><font>{item.productQuantity}</font> {item.status === "webinar" ? "Views" : "Qty"} Remaining</span>
									 </div>
								</div>
						 </Link>
						</div>
...
export default WebinarGridView;
